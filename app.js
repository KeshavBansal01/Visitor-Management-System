...
var transporter = nodemailer.createTransport({
  service: "gmail",
  auth: {
    user: "keshavbansal1.dev@gmail.com", //Enter your E-mail ID and Password
    pass: "keshavbansal",
  },
});

function sndMsg(content, to) {
  //function for sending message using fast2sms api

  var req = unirest("POST", "https://www.fast2sms.com/dev/bulk");

  req.headers({
    authorization:
      "bDLr4JkxpYtW96AGzvXhjUPFKMVSawg2uET5ICNHs1qQRfim7olqua9A1pFcKdRH6oM7V5zQbEhtD20C",
  });
...
function sndMail(content, mailto) {
  //function for sending e-mail using nodemailer
...
function clientCheck(email, callback) {
  //function to check if Visitor is present in Client collection or not
...
function visitCheck(email, callback) {
  //function to check if any visit corresponding to visitor's email
...
function hostCheck(hemail, callback) {
  //function to check if Host is present in Host collection or not
...
function currTime() {
  //function to capture and return current time during checkin and checkout
  var date = new Date();
...
app.post("/checkin", function (req, res) {
  ...
  var add =
    "Innovaccer, Industrial Area, Sector 62, Noida, Uttar Pradesh 201309";
                  ...
                  if (update.length > 0) {
                    Client.find({ email: email }, function (err, update1) {
                      var content =
                        ...
                        update1[update1.length - 1].name +
                        "\nEmail: " +
                        update1[update1.length - 1].email +
                        "\nContact no.: " +
                        update1[update1.length - 1].phone +
                        "\nCheck in Time: " +
                        update[update.length - 1].checkin +
                        "\nCheck out Time: " +
                        dateTime +
                        "\nAddress: " +
                        add;
